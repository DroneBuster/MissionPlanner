            ...
            while (threadrun)
            {
                ...
                try
                {
                    ...
                    if (tracklast.AddSeconds(1.2) < DateTime.Now)
                    {
                        ...
                        if (MainV2.comPort.MAV.cs.ot_fs_status == 1)
                            fs_state.BackColor = Color.Green;
                        else if (MainV2.comPort.MAV.cs.ot_fs_status == 2)
...
// square
        ...
        private void BUT_camoff_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_DIGICAM_CONTROL, 2, 0, 0, 0, 0, 0, 0))
                    CustomMessageBox.Show("The Command failed to execute", "Error");
                MainV2.comPort.setParam("CAM_TRIGG_DIST", 0, true); //TO DO: maybe remove this later when VU supis is upgraded
            }
            catch { CustomMessageBox.Show("The Command failed to execute", "Error"); }
            ((Button)sender).Enabled = true;
        }
        ...
        private void fs_on_Click(object sender, EventArgs e)
        {
            ...
            {
                ((Button)sender).Enabled = false;
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.OT_PARACHUTE, 1, 0, 0, 0, 0, 0, 0))
                    CustomMessageBox.Show("Error sending command", "Error");
            }
            catch { CustomMessageBox.Show("The Command failed to execute", "Error"); }
            ((Button)sender).Enabled = true;
        ...
        private void fs_off_Click(object sender, EventArgs e)
        {
            ...
            {
                ((Button)sender).Enabled = false;
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.OT_PARACHUTE, 2, 0, 0, 0, 0, 0, 0))
                    CustomMessageBox.Show("Error sending command", "Error");
            }
            catch { CustomMessageBox.Show("The Command failed to execute", "Error"); }
            ((Button)sender).Enabled = true;
        ...
        private void land_now_Click(object sender, EventArgs e)
        {
            ...
            if (open_para == DialogResult.Yes)
            {
                ...
                {
                    ((Button)sender).Enabled = false;
                    if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.OT_PARACHUTE, 0, 1, 0, 0, 0, 0, 0))
        ...
        private void gimb_up_Click(object sender, EventArgs e)
        {
            ...
            {
                ((Button)sender).Enabled = false;
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.OT_PARACHUTE, 0, 0, 0, 0, 2, 0, 0))
                    CustomMessageBox.Show("The Command failed to execute", "Error");
        ...
        private void gimb_down_Click(object sender, EventArgs e)
        {
            ...
            {
                ((Button)sender).Enabled = false;
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.OT_PARACHUTE, 0, 0, 0, 0, 1, 0, 0))
                    CustomMessageBox.Show("The Command failed to execute", "Error");
            }
            catch { CustomMessageBox.Show("The Command failed to execute", "Error"); }
            ((Button)sender).Enabled = true;
        ...
        private void para_open_Click(object sender, EventArgs e)
        {
            ...
            {
                ((Button)sender).Enabled = false;
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.OT_PARACHUTE, 0, 0, 0, 2, 0, 0, 0))
                    CustomMessageBox.Show("The Command failed to execute", "Error");
            }
            catch { CustomMessageBox.Show("The Command failed to execute", "Error"); }
            ((Button)sender).Enabled = true;
        ...
        private void para_close_Click(object sender, EventArgs e)
        {
            ...
            {
                ((Button)sender).Enabled = false;
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.OT_PARACHUTE, 0, 0, 0, 1, 0, 0, 0))
                    CustomMessageBox.Show("The Command failed to execute", "Error");
            }
            catch { CustomMessageBox.Show("The Command failed to execute", "Error"); }
            ((Button)sender).Enabled = true;
        ...
        {
            if (MainV2.comPort.BaseStream.IsOpen && MainV2.comPort.MAV.cs.groundspeed > 4)
                if (DialogResult.No == CustomMessageBox.Show("Your model is still moving are you sure you want to turn on ignition (only ignition)?", "Ignition", MessageBoxButtons.YesNo))
                    return;
            try
            {
                ((Button)sender).Enabled = false;
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.OT_PARACHUTE, 0, 0, 1, 0, 0, 0, 0))
                    CustomMessageBox.Show("The Command failed to execute", "Error");
            }
            catch { CustomMessageBox.Show("The Command failed to execute", "Error"); }
            ((Button)sender).Enabled = true;
        ...
        {
            if (MainV2.comPort.BaseStream.IsOpen && MainV2.comPort.MAV.cs.groundspeed > 4)
                if (DialogResult.No == CustomMessageBox.Show("Your model is still moving are you sure you want to turn off ignition (only ignition)?", "Ignition", MessageBoxButtons.YesNo))
                    return;
            try
            {
                ((Button)sender).Enabled = false;
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.OT_PARACHUTE, 0, 0, 2, 0, 0, 0, 0))
                    CustomMessageBox.Show("The Command failed to execute", "Error");
            }
            catch { CustomMessageBox.Show("The Command failed to execute", "Error"); }
            ((Button)sender).Enabled = true;
        ...
        {
            if (MainV2.comPort.BaseStream.IsOpen && MainV2.comPort.MAV.cs.groundspeed > 4)
                if (DialogResult.No == CustomMessageBox.Show("Your model is still moving are you sure you want to reset parachute system?", "System Reset", MessageBoxButtons.YesNo))
                    return;
            try
            {
                ((Button)sender).Enabled = false;
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.OT_PARACHUTE, 42, 0, 0, 0, 0, 0, 0))
                    CustomMessageBox.Show("The Command failed to execute", "Error");
            }
            catch { CustomMessageBox.Show("The Command failed to execute", "Error"); }
            ((Button)sender).Enabled = true;
